...
app.directive('ctree', function() {
	return {
		restrict: 'E',
		template: '<ul class="commentUl"><li class="commentLi" ng-class="{even:depth%2==0, odd:depth%2==1}" ng-repeat="node in tree"><cnode ng-model="node"></cnode></li></ul>',
		replace: true,
		transclude: true,
		scope: {
			tree: '=ngModel', depth: '=',
		...
		link: function(scope, elm, attrs) {
			//if (scope.node.children.length > 0) {
				var child = $compile('<ctree depth="'+(scope.depth+1)+'" ng-model="node.children"'
						...
						+ 'do-linkify="doLinkify({str: str})"></ctree>')(scope)
				elm.append(child);
			//}
		}
	};
...
app.controller('BoardController', function($http, $scope, $location, $route, $routeParams, $window) {
	...
	$scope.Linkify = function(htmlString) {
		if (htmlString)
    		return htmlString.replace(/(\b(https?|ftp|file):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/ig,"<a href='$1'>$1</a>"); 
	}
	...
	function GetPostList(pageNo) {
		...
		var url = 'backend/get.php?t=1&boardName='+$scope.boardName;
