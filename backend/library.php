...
define('publicUrl','http://uwksa.org');
...
function GetPostList($board_Id, $pId, $pNo, $PPP, $tag, $searchParam, $searchOptions) {						// Posts Per Page
	...
	// Fill in $returnList
		for ($i=0; $i<sizeof($rawList); $i++) {
			...
			/* $pureContent = htmlentities(strcut_utf8(str_replace("&", "&amp;",str_replace('&nbsp;', ' ', htmlspecialchars_decode(strip_tags(preg_replace("/<p/", " <p", preg_replace("/<div/", " <div", preg_replace("/<img(?:.|\n)*?>/", " [ attached image ] ", $rawList[$i]['content_original']))))))), 200)
			 							, ENT_QUOTES
			 							, "UTF-8");*/
			$pureContent = strcut_utf8(str_replace("&", "&amp;",str_replace('&nbsp;', ' ', htmlspecialchars_decode(strip_tags(preg_replace("/<p/", " <p", preg_replace("/<div/", " <div", preg_replace("/<img(?:.|\n)*?>/", " [ attached image ] ", $rawList[$i]['content_original']))))))), 200);
			if ($i == 0) {
				$firstContent = $htmlContent;
				$firstPureContent = $pureContent;
			}
			...
			$returnList[$i]['canEdit'] = $rawList[$i]['user_id']==$_SESSION['UserID'] || $_SESSION['UserLevel'] == admin_level;
			$returnList[$i]['canDelete'] = $rawList[$i]['user_id']==$_SESSION['UserID'] || $_SESSION['UserLevel'] == admin_level;
			$returnList[$i]['canComment'] = $_SESSION['UserLevel'] <= new_user_level;
			$returnList[$i]['authorUserId'] = $rawList[$i]['user_id'];
			$returnList[$i]['author'] = htmlentities($rawList[$i]['user_name'], ENT_QUOTES, "UTF-8");
			$returnList[$i]['authorEmail'] = $rawList[$i]['user_email'];
			$returnList[$i]['facebookId'] = $rawList[$i]['facebook_user_id'];
			$returnList[$i]['title'] = str_replace("&", "&amp;", $rawList[$i]['title']);
			...
			if ($chk && sizeof($returnList) > 0) {
				$returnList[0]['content'] = $firstContent;
				$returnList[0]['pureContent'] = $firstPureContent;
			}
			$returnList[$i]['commentCount'] = $rawList[$i]['comments'];
			
			$returnList[$i]['files'] = DBGetPostFiles($returnList[$i]['postId']);
...
function GetPostListTest($board_Id, $pId, $pNo, $PPP, $tag, $searchParam, $searchOptions) {						// Posts Per Page
	If ($pId == null || $pId == '') $pId = -1;
	echo "board=" . $board_Id . " postId=" . $pId . " pageNo=" . $pNo . " PPP=" . $PPP . " tags=" . $tag . " searchParam= " . $searchParam . " searchOptions=" . $searchOptions;
	...
	// Fill in $returnList
		for ($i=0; $i<sizeof($rawList); $i++) {
			...
			$htmlContent = $rawList[$i]['content_original'];
			$htmlContentShortened =strcut_utf8($htmlContent, 1000);
			 /*$pureContent = htmlentities(strcut_utf8(strip_tags(preg_replace("/<p/", " <p", preg_replace("/<div/", " <div", preg_replace("/<img(?:.|\n)*?>/", " [ attached image ] ", $rawList[$i]['content_original'])))), 200)
			 							, ENT_QUOTES
			 							, "UTF-8");*/
			$pureContent = strcut_utf8(str_replace("&", "&amp;",str_replace('&nbsp;', ' ', htmlspecialchars_decode(strip_tags(preg_replace("/<p/", " <p", preg_replace("/<div/", " <div", preg_replace("/<img(?:.|\n)*?>/", " [ attached image ] ", $rawList[$i]['content_original']))))))), 200);
			if ($i == 0) {
...
function GetPostContent($pId) {
	...
	$result['files'] = DBGetPostFiles($pId);
...
function DBGetPostList($bName, $PPP, $PId, $pNo, &$TotalPostNum, &$CurrentPage, $Tags, $searchParam, $searchOptions) {
	...
	if ($stmt->prepare('CALL Get_Post_List(?, ?, ?, ?, ?, ?)')) {
		...
		$stmt->execute();
		$stmt->bind_result($post['post_no'], $post['user_id'], $post['user_name'], $post['user_email'], $post['facebook_user_id'], $post['title'], $post['content_original'], $post['comments'], $post['pinned'], $post['date']);
		while ($stmt->fetch()) {
...
function DBGetCommentFiles($commentId) {
	global $db;
	$db->next_result();
	$stmt = $db->stmt_init();
	$index = 0;
	if ($stmt->prepare('CALL Get_Comment_Files(?)')) {
		$stmt->bind_param ('i', $commentId);
		$stmt->execute();
		$stmt->bind_result($uploadfile['name'], $uploadfile['address']);
		while($stmt->fetch()){
		 $file_list[$index++] = unserialize(serialize($uploadfile));
		 }
		$stmt->close();
	}
...
function DBGetPostFiles($postId) {
	global $db;
	$db->next_result();
	$stmt = $db->stmt_init();
	$index = 0;
	if ($stmt->prepare('CALL Get_Post_Files(?)')) {
		$stmt->bind_param ('i', $postId);
		$stmt->execute();
		$stmt->bind_result($uploadfile['name'], $uploadfile['address'], $uploadfile['size']);
		while($stmt->fetch()){
			$uploadfile['size'] = $uploadfile['size'] > 1048576 ? round($uploadfile['size']/1048576, 1).' MB' : round($uploadfile['size']/1024, 1).' KB';
			$file_list[$index++] = unserialize(serialize($uploadfile));
		}
		$stmt->close();
	}
...
function DBGetUserComments($parentPost) {
	...
	if($stmt->prepare('CALL Get_Post_Comment(?)')) {
		...
		$stmt->execute();
		$stmt->bind_result($comment['deleted'], $comment['parentComment'], $comment['comment_no'], $comment['user_id'], $comment['user_name'], $comment['user_email'], $comment['facebook_user_id'], $comment['content_original'], $comment['date']);
		while($stmt->fetch()){
...
function DBGetPostContent($pId) {
	...
	if($stmt->prepare('CALL Get_Post_Content(?)')) {
		...
		$stmt->execute();
		$stmt->bind_result($post['post_id'], $post['user_id'], $post['board_id'],  $post['user_name'], $post['user_email'], $post['facebook_user_id'], $post['date'], $post['title'], $post['content'], $post['comments'], $post['pinned']);
		$stmt->fetch();
		$stmt->close();
		return $post;
	}
...
function DBSaveComment($cid, $parentPost, $parentComment, $content, $user_id, $deleted, $addedFiles, $deletedFiles) {
	$date = getTime();
	global $db;
	$stmt = $db->stmt_init();
			...
			if($stmt->prepare('CALL Save_User_Comment(?, ?, ?, ?, ?, ?,?,?)')) {
				...
				if ($commentId > 0) {
					...
					if (sizeof($addedFiles) > 0) {
						...
						while ($file = $addedFiles[$index++]['file']) {
							...
							//DB Save
							$result = DBSaveUploadFile($fileextension, -1, $commentId, $filesize, $fileDirectory, $fileAddress, $filename);
							...
							foreach ($tags as $tag) {
								...
								if (strpos($source, $filealias) !== false) {
							...
							$content = htmlentities($newContent, ENT_QUOTES, "UTF-8");
							$db->next_result();
							$stmt = $db->stmt_init();
							//htmlspecialchars($title, ENT_HTML401, 'UTF-8', false)
							if($stmt->prepare('CALL Save_User_Comment(?, ?, ?, ?, ?, ?,?,?)')) {
								$stmt->bind_param('iiissiii', $commentId, $parentPost, $parentComment, $user_id, $newContent, $deleted, $date, $date);
								$stmt->execute();
				...
				if ($deletedFiles != null) {
					foreach ($deletedFiles as $filename ) {
						...
						$db->next_result();
						$stmt = $db->stmt_init();
						if ($stmt->prepare('CALL Delete_Comment_File(?, ?)')) {
				...
				$returnArray['canDelete'] = true;
				$returnArray['canComment'] = true;
				$returnArray['authorUserId'] = $comment['user_id'];
				$returnArray['author'] = htmlentities($comment['user_name'], ENT_QUOTES, "UTF-8");
				...
				$returnArray['files'] = DBGetCommentFiles($comment['comment_no']);
			}
		}
	}
...
function DBSavePost($post_id, $pinned, $boardName, $title, $content, $user_id, $added_tags, $deleted_tags, $addedFiles, $deletedFiles) {
	...
	global $db;
	$stmt = $db->stmt_init();
	if ($pinned && $_SESSION['UserLevel'] > exec_level) $pinned = false;
...
function DBSaveUploadFile ($file_extension, $post_id, $comment_id, $file_size, $file_directory, $file_address, $filename) {
	global $db;
	$stmt = $db->stmt_init();
	$db->next_result();
	$sql = 'CALL Save_Upload_File(?, ?, ?, ?, ?, ?, ?)';
	if($stmt->prepare($sql)) {
		$stmt->bind_param('siiisss', $file_extension, $post_id, $comment_id, $file_size, $file_directory, $file_address, $filename);
		$stmt->execute();
...
function DBUpdateForgottenPassword($email, $password){
	global $db;
	$db ->next_result();
	$stmt = $db->stmt_init();
	$sql = 'CALL Save_Forgotten_Password(?,?)';
	if($stmt->prepare($sql)) {
		$stmt->bind_param('ss', $email, $password);
		$stmt->execute();
		$stmt->close();
...
function DBDeletePost($pId, $user_id, $files) {
	global $db;
	$stmt = $db->stmt_init();
	if ($stmt->prepare('CALL Check_Post_Owner(?,?)')) {
		...
		if ($result == 0 || $_SESSION['UserLevel'] == admin_level) {
			if ($files != null) {
				foreach ($files as $filename ) {
					$encoded_post_id = str_replace("/", "SLASH", fnEncrypt("p".$pId));
					$target_dir = "../upload/".$encoded_post_id."/".$filename;
					if(file_exists($target_dir)) {
						unlink($target_dir);
						@rmdir("../upload/".$encoded_post_id."/");
					} 
				}
			}
...
function format_date($date) {
	...
	if ($delta < 1) {
		...
		$format[1] = $delta . ' seconds ago';
	} else if($delta < 120) {
		$format[1] = 'a minute ago';
	} else if($delta < (45*60)) {
		$format[1] = floor($delta/60) . ' minutes ago';
	} else if($delta < (2*60*60)) {
		$format[1] = 'an hour ago';
	} else if($delta < (24*60*60)) {
		$format[1] = '' + floor($delta/3600) . ' hours ago';
